...
} from "../../../../core/src/domain/file/file-types";
import type {
  FileWorkspaceEntry,
...
interface DirState {
  ...
  depth: number;
  expanded: boolean;
  children: FileWorkspaceEntry[] | null; // null = not yet fetched
  loading: boolean;
  error?: string;
}

export default function FileTree({ gateway, onOpenFile, selectedPath, ready, cwd, followCwd, initialRoot, initialFollow = true, onTreeStateChange, revealRequest }: Props) {
  const rpc = gateway.request;
  const onEvent = gateway.subscribe;
  ...
  const fetchDir = useCallback(
    async (dirPath: string, depth: number): Promise<DirState> => {
      ...
      rerender();
      try {
        const res = await rpc<{ entries: FileWorkspaceEntry[] }>("fs:list", { path: dirPath });
  ...
  useEffect(() => {
    return onEvent("fs:event", (msg: FileWorkspaceEventMessage) => {
  ...
  const matchesLoadedTree = (parentPath: string, entry: FileWorkspaceEntry): boolean => {
  ...
  const renderEntries = (parentPath: string, entries: FileWorkspaceEntry[], depth: number): React.ReactNode =>
      ...
      if (child.dir) {
        ...
        return (
          <div key={full}>
            ...
            {expanded &&
              (st!.error ? (
                <div className="tree-error" role="alert" style={{ paddingLeft: 22 + depth * 12 }}>
                  {st!.error}
                </div>
              ) : st!.children ? (
  ...
  return (
    <div className="tree-scroll" ref={treeScrollRef}>
      ...
      {rootSt?.expanded &&
        (rootSt.error ? (
          <div className="tree-error" role="alert">{rootSt.error}</div>
        ) : rootSt.children ? (
