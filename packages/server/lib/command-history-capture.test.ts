...
describe("shell command history capture contract", () => {
  it("reports commands from precmd together with their exit status", () => {
    expect(wsServerSource).toContain("precmd_functions=(__ca_hist_precmd $precmd_functions)");
    expect(wsServerSource).toContain("preexec_functions=(__ca_hist_preexec $preexec_functions)");
    expect(wsServerSource).toContain('function __ca_hist_precmd(){ local e=$?; if [[ -n "\\${__ca_hist_cmd+x}" ]]; then');
    expect(wsServerSource).toContain(String.raw`printf '\\033]633;C;%s;%s;%s\\007' "$c" "$d" "$e"`);
    expect(wsServerSource).toContain("unset __ca_hist_cmd");
  });
