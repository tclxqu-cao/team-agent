...
import { createPinnedCommand } from "./pinnedCommands";
...
export default function HistoryPanel({csrfToken,terminalId,pinnedCommands,onPinnedCommandsChange,onFill,onExecute}:{csrfToken:string;terminalId:string|null;pinnedCommands:PinnedCommand[];onPinnedCommandsChange:(commands:PinnedCommand[])=>void;onFill:(command:string)=>void;onExecute:(command:string)=>void}){
  ...
  const pin=useCallback((command:string)=>{if(command==="[REDACTED]"||pinnedCommands.some(item=>item.command===command))return;onPinnedCommandsChange([...pinnedCommands,createPinnedCommand(command)]);},[onPinnedCommandsChange,pinnedCommands]);
  const copy=async(command:string)=>{const copied=await copyTextToClipboard(command);setCopyStatus(copied?"已复制命令":"复制失败，请长按命令手动复制");};
  return <div className="history-panel">
    ...
    {copyStatus&&<div className="terminal-clipboard-notice" role="status">{copyStatus}</div>}
    {items.map(item=><div className={`history-item${terminalId?"":" history-item-disabled"}`} key={item.id} role="button" tabIndex={terminalId?0:-1} onClick={()=>fill(item.command)} onKeyDown={(event)=>{if(event.target===event.currentTarget&&(event.key==="Enter"||event.key===" ")&&terminalId){event.preventDefault();fill(item.command);}}}><code>{item.command}</code><small>{item.cwd} · {new Date(item.executedAt).toLocaleString()}</small><div className="history-item-actions" onClick={(event)=>event.stopPropagation()} onPointerDown={(event)=>event.stopPropagation()}>
      <button className="history-icon-button" onClick={()=>void copy(item.command)} aria-label="复制命令" title="复制"><Copy size={12} aria-hidden="true"/></button>
