...
The conversation removes avatars below 900px and uses role-specific message modifiers. User bubbles remain compact and right-aligned at no more than 88% width. Assistant responses use the available width and a quiet surface boundary for readable code, tools, and long answers.

The composer is the signature element: a fixed visual dock with a slim context ribbon across its top edge. Attachment and send controls are 44px circles, the text field remains 16px to prevent iOS zoom, and desktop-only keyboard help is hidden. The dock remains above `safe-area-inset-bottom` and the browser keyboard.
