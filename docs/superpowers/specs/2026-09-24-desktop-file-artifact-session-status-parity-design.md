...
- 点击后从右侧打开与 Web 一致的“文件 / 历史”抽屉；再次点击或点击关闭按钮收起。
- 桌面宽屏同时显示聊天、文件树和预览；宽度不足时预览覆盖在抽屉之上，关闭预览后返回文件树。固定网格尺寸保证开关和加载状态不推动主布局。
...
- 按项目规则用 ego-browser 验证 Web 桌面视口与窄视口：文件/历史抽屉、预览和交互物链接无回归。
- 启动真实 Electron，验证文件图标、抽屉、HTML/Markdown/图片/PDF 预览、文本编辑与一次越界拒绝。
