...
- Existing Google embedded-login interception remains active when imported authentication is absent or expired.
...
4. Sites with valid source sessions remain logged in; expired sessions use the safe external-login fallback.
...
7. Logs, IPC payloads, and metadata contain no cookie values or secrets.

## Scope

This phase supports macOS, the detected `Default` Profile for Chrome and ego-lite, one active imported snapshot, and one shared AI Hub browser identity. Multiple named browser Profiles, Windows/Linux import, extension loading, password-manager migration, Chrome Sync, automatic periodic synchronization, and importing while the source browser is running are out of scope.
